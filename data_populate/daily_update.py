#!/usr/bin/env python3
"""
Daily Update Script for Historical Data
Updates existing NIFTY 500 stocks and indices with latest daily data
"""
...
def ingest_candles_df(df_candles, instrument_token, tradingsymbol, exchange='NSE', interval='1day'):
    ...
    cur = conn.cursor()
    try:
        # Create temp table (drop if exists first to be safe)
        cur.execute("DROP TABLE IF EXISTS temp_ohlcv")
        cur.execute("""
...
def main():
    ...
    logger.info(f"Update period: {START_DATE.date()} -> {END_DATE.date()}")

    # Read NIFTY 500 stocks
    nifty500_file = "nifty500.txt"
    nifty500_list = []
    try:
        with open(nifty500_file, 'r') as f:
            nifty500_list = [line.strip() for line in f if line.strip()]
        logger.info(f"Loaded {len(nifty500_list)} stocks from {nifty500_file}")
    except FileNotFoundError:
        logger.warning(f"NIFTY 500 file {nifty500_file} not found - skipping stocks")
    ...
    try:
        ...
        logger.warning(f"Indices file {indices_file} not found - skipping indices")

    # Combine all symbols
    all_symbols = nifty500_list + indices_list
    if not all_symbols:
        logger.error("No symbols to process!")
        return

    logger.info(f"Total symbols to update: {len(all_symbols)} ({len(nifty500_list)} stocks + {len(indices_list)} indices)")

    # Get instruments for the specified symbols
    if all_symbols:
        symbols_placeholder = ', '.join(['%s'] * len(all_symbols))
        query = f"""
        ...
        with engine.connect() as conn:
            df_candidates = pd.read_sql(query, conn, params=tuple(all_symbols))

        logger.info(f"Found {len(df_candidates)}/{len(all_symbols)} symbols in database")

        if len(df_candidates) < len(all_symbols):
            missing = set(all_symbols) - set(df_candidates['tradingsymbol'].tolist())
            logger.warning(f"Missing symbols: {', '.join(list(missing)[:10])}{'...' if len(missing) > 10 else ''}")
    else:
    ...
    with tqdm(total=total_tasks, desc="Daily Update Progress", position=0) as pbar_overall:
        for idx, r in df_candidates.iterrows():
            tkn = int(r['instrument_token'])
            sym = r['tradingsymbol']

            logger.info(f"\nProcessing: {sym} ({tkn}) - {idx+1}/{len(df_candidates)}")

            for itv in INTERVALS:
                chunk_days = CHUNK_DAYS_BY_INTERVAL.get(itv, 90)

                # Calculate chunks for this short period
                total_days = (END_DATE - START_DATE).days
                total_chunks = max(1, total_days // chunk_days)

                logger.info(f"{sym} - {itv}: updating ({START_DATE.date()} -> {END_DATE.date()})")
                ...
                with tqdm(total=total_chunks, desc=f"  {itv}", position=1, leave=False) as pbar_interval:
                    res = process_instrument_interval(
                        ...
                        tradingsymbol=sym,
                        interval=itv,
                        start_ts=START_DATE,  # Fixed start date for daily updates
                        end_ts=END_DATE,      # Current date
                ...
                time.sleep(2.0)  # Rate limiting between symbols
    ...
    logger.info(f"\nTotal symbols processed: {len(df_candidates)}")
    logger.info(f"Total intervals: {len(INTERVALS)}")
    logger.info(f"Total rows inserted: {df_summary['inserted'].sum():,}")
    logger.info(f"Successful updates: {len(df_summary[df_summary['status'] == 'done'])}")
    logger.info(f"Failed updates: {len(df_summary[df_summary['status'] == 'error'])}")

    # Save summary to CSV
    summary_file = f"daily_update_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    df_summary.to_csv(summary_file, index=False)
    logger.info(f"\nDetailed summary saved to: {summary_file}")

    # Show top performers
    if not df_summary.empty and df_summary['inserted'].sum() > 0:
        logger.info("\nTop 5 symbols by rows inserted:")
        top5 = df_summary.nlargest(5, 'inserted')[['symbol', 'interval', 'inserted']]
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"\n\nFatal error in daily update: {e}", exc_info=True)
        sys.exit(1)
